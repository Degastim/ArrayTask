...
public class JaggedArray {
    ...
    public boolean equals(Object o) {
        ...
        int[][] arrayObject = ((JaggedArray) o).getValues();
        int arrayLength = arrayObject.length;
        int valuesLength = values.length;
        if (valuesLength != arrayLength) return false;
        int i = 0, j = 0;
        while (i < values.length) {
            if (values[i].length != arrayObject[i].length) return false;
            while (j < values[i].length) {
                if (values[i][j] != arrayObject[i][j]) {
                    return false;
                }
                j++;
            }
